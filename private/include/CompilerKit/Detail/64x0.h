...
#define NC_ASM_OPCODE(__NAME, __OPCODE, __FUNCT3, __FUNCT7) \
...
inline std::vector<CpuOpcode64x0> kOpcodes64x0 = {
    NC_ASM_OPCODE("nop", 0b0000000, 0b0000000, kAsmNoArgs)  // no-operation.
    NC_ASM_OPCODE("np", 0b0000000, 0b0000000, kAsmNoArgs)   // no-operation.
    NC_ASM_OPCODE("jlr", 0b1110011, 0b0000111,
                  kAsmJump)  // jump to linked return register
    NC_ASM_OPCODE("jrl", 0b1110011, 0b0001111,
                  kAsmJump)  // jump from return register.
    NC_ASM_OPCODE("mv", 0b0100011, 0b101, kAsmRegToReg) NC_ASM_OPCODE(
        "bg", 0b1100111, 0b111, kAsmRegToReg) NC_ASM_OPCODE("bl", 0b1100111, 0b011, kAsmRegToReg)
        NC_ASM_OPCODE("beq", 0b1100111, 0b000, kAsmRegToReg)
            NC_ASM_OPCODE("bne", 0b1100111, 0b001, kAsmRegToReg)
                NC_ASM_OPCODE("bge", 0b1100111, 0b101, kAsmRegToReg)
                    NC_ASM_OPCODE("ble", 0b1100111, 0b100, kAsmRegToReg)
                        NC_ASM_OPCODE("stw", 0b0001111, 0b100, kAsmImmediate)
                            NC_ASM_OPCODE("ldw", 0b0001111, 0b100, kAsmImmediate)
                                NC_ASM_OPCODE("lda", 0b0001111, 0b101, kAsmImmediate)
                                    NC_ASM_OPCODE("sta", 0b0001111, 0b001, kAsmImmediate)
    // add/sub without carry flag
    NC_ASM_OPCODE("add", 0b0101011, 0b100, kAsmImmediate)
        NC_ASM_OPCODE("sub", 0b0101011, 0b101, kAsmImmediate)
    // add/sub with carry flag
    NC_ASM_OPCODE("addc", 0b0101011, 0b110, kAsmImmediate) NC_ASM_OPCODE(
        "subc", 0b0101011, 0b111, kAsmImmediate) NC_ASM_OPCODE("sc", 0b1110011, 0b00, kAsmSyscall)};
