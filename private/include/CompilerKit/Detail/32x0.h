...
#ifndef NCC_COMPILERKIT_DETAIL_32X0_H
#define NCC_COMPILERKIT_DETAIL_32X0_H
...
inline std::vector<CpuCode32x0> kOpcodes32x0 = {
    NC_ASM_OPCODE("nop", 0b0100011, 0b000, kAsmNoArgs)     // nothing to do. (1C)
    NC_ASM_OPCODE("jmp", 0b1110011, 0b001, kAsmJump)       // jump to branch (2C)
    NC_ASM_OPCODE("mov", 0b0100011, 0b101, kAsmImmediate)  // move registers (3C)
    NC_ASM_OPCODE("psh", 0b0111011, 0b000, kAsmImmediate)  // push to sp (2C)
    NC_ASM_OPCODE("pop", 0b0111011, 0b001, kAsmImmediate)  // pop from sp. (1C)
    NC_ASM_OPCODE("lea", 0b0111011, 0b010,
                  kAsmImmediate)  // setup stack and call, store address to CR (1C).
    NC_ASM_OPCODE("ret", 0b0111011, 0b110,
                  kAsmImmediate)                         // return from procedure (2C).
    NC_ASM_OPCODE("uc", 0b0111111, 0b000, kAsmSyscall)   // user call (1C)
    NC_ASM_OPCODE("kc", 0b0111111, 0b001, kAsmSyscall)   // kernel call (1C)
    NC_ASM_OPCODE("int", 0b0111111, 0b010, kAsmSyscall)  // raise interrupt (1C)
};
...
#endif  // NCC_COMPILERKIT_DETAIL_32X0_H
